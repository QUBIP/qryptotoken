...
use slh_dsa::{
    signature::Keypair, Shake128s, Signature as SlhDsaSignature, SigningKey,
    VerifyingKey,
};

#[derive(Clone)]
pub struct PubKey(Box<VerifyingKey<Shake128s>>);

#[derive(Clone)]
pub struct PrivKey(Box<SigningKey<Shake128s>>);

pub struct Signature(Box<SlhDsaSignature<Shake128s>>);

pub mod sizes {
    #![allow(dead_code)]
    use super::*;

    pub(crate) const SLH_DSA_SHAKE_128S_PK_SIZE: usize = PubKey::output_len();
    pub(crate) const SLH_DSA_SHAKE_128S_SK_SIZE: usize = PrivKey::output_len();
    pub(crate) const SLH_DSA_SHAKE_128S_SIG_SIZE: usize =
...
impl Verifier<Signature> for PubKey {
    fn verify(
        ...
        VerifyingKey::<Shake128s>::try_verify_with_context(
            &self.0,
            msg,
            &[],
            &signature.0,
        ...
        .map_err(|e| {
            signature::Error::from_source(AdapterError::VerificationError(
                format!(
                    "SLH-DSA-SHAKE128s signature verification failed: {e:?}"
                ),
            ))
        })?;
...
impl Signer<Signature> for PrivKey {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        ...
        let sig = SigningKey::<Shake128s>::try_sign_with_context(
            &self.0,
            msg,
            &[],
            Some(&rnd),
        ...
        .map_err(|e| {
            signature::Error::from_source(AdapterError::SigningError(format!(
                "SLH-DSA-SHAKE128s signing failed: {e:?}"
            )))
        })?;
...
/// Generates an SLH-DSA-SHAKE128s key pair.
///
/// # Returns
///
...
pub fn generate_key_pair() -> AResult<(PrivKey, PubKey)> {
    ...
    let privkey = SigningKey::<Shake128s>::new(&mut rng);
