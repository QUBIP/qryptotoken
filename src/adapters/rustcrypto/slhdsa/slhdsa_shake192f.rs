...
use slh_dsa::{
    signature::Keypair, Shake192f, Signature as SlhDsaSignature, SigningKey,
    VerifyingKey,
};

#[derive(Clone)]
pub struct PubKey(Box<VerifyingKey<Shake192f>>);

#[derive(Clone)]
pub struct PrivKey(Box<SigningKey<Shake192f>>);

pub struct Signature(Box<SlhDsaSignature<Shake192f>>);

pub mod sizes {
    #![allow(dead_code)]
    use super::*;

    pub(crate) const SLH_DSA_SHAKE_192F_PK_SIZE: usize = PubKey::output_len();
    pub(crate) const SLH_DSA_SHAKE_192F_SK_SIZE: usize = PrivKey::output_len();
    pub(crate) const SLH_DSA_SHAKE_192F_SIG_SIZE: usize =
...
impl Verifier<Signature> for PubKey {
    fn verify(
        ...
        VerifyingKey::<Shake192f>::try_verify_with_context(
            &self.0,
            msg,
            &[],
            &signature.0,
        ...
        .map_err(|e| {
            signature::Error::from_source(AdapterError::VerificationError(
                format!(
                    "SLH-DSA-SHAKE192f signature verification failed: {e:?}"
                ),
            ))
        })?;
...
impl Signer<Signature> for PrivKey {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        ...
        let sig = SigningKey::<Shake192f>::try_sign_with_context(
            &self.0,
            msg,
            &[],
            Some(&rnd),
        ...
        .map_err(|e| {
            signature::Error::from_source(AdapterError::SigningError(format!(
                "SLH-DSA-SHAKE192f signing failed: {e:?}"
            )))
        })?;
...
/// Generates an SLH-DSA-SHAKE192f key pair.
///
/// # Returns
///
...
pub fn generate_key_pair() -> AResult<(PrivKey, PubKey)> {
    ...
    let privkey = SigningKey::<Shake192f>::new(&mut rng);
