...
use slh_dsa::{
    signature::Keypair, Shake128f, Signature as SlhDsaSignature, SigningKey,
    VerifyingKey,
};

#[derive(Clone)]
pub struct PubKey(Box<VerifyingKey<Shake128f>>);

#[derive(Clone)]
pub struct PrivKey(Box<SigningKey<Shake128f>>);

pub struct Signature(Box<SlhDsaSignature<Shake128f>>);

pub mod sizes {
    #![allow(dead_code)]
    use super::*;

    pub(crate) const SLH_DSA_SHAKE_128F_PK_SIZE: usize = PubKey::output_len();
    pub(crate) const SLH_DSA_SHAKE_128F_SK_SIZE: usize = PrivKey::output_len();
    pub(crate) const SLH_DSA_SHAKE_128F_SIG_SIZE: usize =
...
impl Verifier<Signature> for PubKey {
    fn verify(
        ...
        VerifyingKey::<Shake128f>::try_verify_with_context(
            &self.0,
            msg,
            &[],
            &signature.0,
        ...
        .map_err(|e| {
            signature::Error::from_source(AdapterError::VerificationError(
                format!(
                    "SLH-DSA-SHAKE128f signature verification failed: {e:?}"
                ),
            ))
        })?;
...
impl Signer<Signature> for PrivKey {
    fn try_sign(&self, msg: &[u8]) -> Result<Signature, signature::Error> {
        ...
        let sig = SigningKey::<Shake128f>::try_sign_with_context(
            &self.0,
            msg,
            &[],
            Some(&rnd),
        ...
        .map_err(|e| {
            signature::Error::from_source(AdapterError::SigningError(format!(
                "SLH-DSA-SHAKE128f signing failed: {e:?}"
            )))
        })?;
...
impl PrivKey {
    ...
    pub fn try_sign_with_ctx(
        ...
        let rnd_opt: Option<[u8; 16]> = if det {
            None
        } else {
            let mut rnd = [0u8; 16];
...
/// Generates an SLH-DSA-SHAKE128f key pair.
///
/// # Returns
///
...
pub fn generate_key_pair() -> AResult<(PrivKey, PubKey)> {
    ...
    let privkey = SigningKey::<Shake128f>::new(&mut rng);
